...
MAX_DAYS   = 5  # 🟢 往期展示最多保留天数（修改此处实现自动清理）
...
def inject_to_html(all_days: List[Dict], current_date: str):
    ...
    print(f"[✅] 已成功注入 {len(all_days)} 天数据（最近5天），当前日期: {current_date}")
    return True
...
def main():
    ...
    try:
        ...
        # ========== 关键修改 ==========
        # 取最新数据的日期作为 currentDate，而非传入的 date_str
        latest_date = all_days[0]["date"]
        # ========== 关键修改 ==========

        # 执行注入（传入 latest_date 而非 date_str）
        if inject_to_html(all_days, latest_date):
            # 优化日志：提示最新数据日期
            print(f"[INFO] 注入口令的当前日期已修正为最新数据日期: {latest_date}")
            sys.exit(0)
        else:
            sys.exit(1)
